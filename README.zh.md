...
- **接管官方侧边栏** — 从会话、侧边栏文件列表或源代码管理打开的每个文件都落进本插件的编辑器视图：只要本插件能读，官方预览面板就不会接手。
...
- **文件搜索** — 按文件名实时搜索整个工作区。
- **文件编辑器** — 会话视图「编辑器」页签下的多标签编辑器：打开任意文本文件、编辑并保存（按钮或 Ctrl+S）；图片以缩放/平移预览打开；`.md`/`.markdown` 与 `.html` 以渲染预览打开，可在预览/源码间切换；`.pdf` 就地渲染；其余二进制文件会被识别并拒绝打开，避免损坏。
